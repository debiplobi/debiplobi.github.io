			...
			<main className="max-w-3xl w-full flex flex-col gap-24">
				...
				<section className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-6">
					...
					<p className="text-base sm:text-lg text-muted-foreground/80 leading-relaxed max-w-2xl">
						...
						problems. I build websites, APIs, command-line tools, scripts,
					...
					<div className="flex flex-wrap gap-4 pt-4">
						<Button
							size="lg"
							className="rounded-full shadow-sm hover:shadow-md transition-[background-color,box-shadow,border-color]"
						>
							View Work
						</Button>
						<Button
							size="lg"
							variant="outline"
							className="rounded-full hover:bg-muted/50 transition-[background-color,box-shadow,border-color]"
						>
							Get in touch
						</Button>
					</div>
				...
				<section className="animate-in fade-in slide-in-from-bottom-6 duration-1000 delay-100 fill-mode-both space-y-6">
					...
					<div className="flex flex-wrap gap-3">
						{[
							...
							<span
								key={tool}
								className="px-4 py-2 bg-muted text-muted-foreground hover:bg-primary hover:text-primary-foreground hover:shadow-sm cursor-default transition-[background-color,color,box-shadow] duration-300 rounded-full text-sm font-medium border border-border/50"
							>
								{tool}
							</span>
						))}
						...
						{recentProjects.length > 0 ? (
							recentProjects.map((project) => (
								<a
									...
									className="group relative rounded-2xl border border-border/50 bg-card p-6 overflow-hidden hover:border-border/80 hover:shadow-sm transition-[background-color,box-shadow,border-color] duration-300"
								>
						...
						{recentBlogs.length > 0 ? (
							recentBlogs.map((blog) => (
								<a
									...
									className="group relative rounded-2xl border border-border/50 bg-card p-6 overflow-hidden hover:border-border/80 hover:shadow-sm transition-[background-color,box-shadow,border-color] duration-300 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
								>
					...
					<div className="flex flex-col gap-4">
						...
						<div className="rounded-2xl border border-border/50 bg-card p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
							<div className="space-y-1">
								...
								<p className="text-sm text-muted-foreground/80">
									Global Institute of Science & Technology, Haldia
								</p>
							</div>
							<span className="text-xs font-mono text-muted-foreground bg-muted px-2 py-1 rounded-md">
								76%
				...
				<footer className="pt-12 border-t border-border/30 flex flex-col sm:flex-row items-center justify-between gap-4 animate-in fade-in duration-1000 delay-300 fill-mode-both mt-12">
					<p className="text-sm text-muted-foreground">
						&copy; {new Date().getFullYear()} Debiprasad Das. All rights
						reserved.
					</p>
					<div className="flex items-center gap-2">
						<a
							href="https://github.com/debiplobi"
							target="_blank"
